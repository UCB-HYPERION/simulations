...
class TestAbsorber(unittest.TestCase):
    def setUp(self):
        ...
        self.dB = -15.
        self.ang = np.pi / 6
        self.abs = absorber.Absorber(self.ang)
...
class TestBeamAbsorber(unittest.TestCase):
    def setUp(self):
        ...
        self.smooth = 1e-6
        self.ang = np.pi / 4
        self.bm = absorber.BeamAbsorber(self.fq, self.ang)
    def test_attributes(self):
    ...
    def test_response(self):
        ...
        ang2 = self.ang - 0.01
        xyz = np.array([0,np.cos(ang1),np.sin(ang1)])
        np.testing.assert_equal(self.bm.response(xyz, self.smooth, data_file=False, flat=dB, use_abs=True),1)
        xyz = np.array([0,np.cos(ang2),np.sin(ang2)])
        np.testing.assert_equal(self.bm.response(xyz, self.smooth, data_file=False, flat=dB, use_abs=True),10**(-15/20.))
