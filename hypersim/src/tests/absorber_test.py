...
class TestAbsorber(unittest.TestCase):
    ...
    def test_abs_response(self):
        xyz = np.array([0,0,1])
        self.assertEqual(self.abs.abs_response(xyz, self.fq, self.smooth, absorber=self.dB),1)
        xyz = np.array([0,1,0])
        self.assertEqual(self.abs.abs_response(xyz, self.fq, self.smooth, absorber=self.dB),10**(self.dB/20.))
        ang1 = self.ang + 0.01
        ang2 = self.ang - 0.01
...
class TestBeamAbsorber(unittest.TestCase):
    ...
    def test_response(self):
        ...
        no_abs = self.bm.response(top, self.smooth, absorber=dB, use_abs=False)
        abs_0dB = self.bm.response(top, self.smooth, absorber=dB, use_abs=True)
        np.testing.assert_equal(no_abs, abs_0dB)
        dB = -15.
        abs_15dB = self.bm.response(top, self.smooth, absorber=dB, use_abs=True)
        self.assertFalse(np.all(no_abs == abs_15dB))
        ang1 = self.ang + 0.01
        ang2 = self.ang - 0.01
        xyz = np.array([0,np.cos(ang1),np.sin(ang1)])
        np.testing.assert_equal(self.bm.response(xyz, self.smooth, absorber=dB, use_abs=True),self.bm.beam_response(xyz))
        xyz = np.array([0,np.cos(ang2),np.sin(ang2)])
        np.testing.assert_equal(self.bm.response(xyz, self.smooth, absorber=dB, use_abs=True),self.bm.beam_response(xyz)*10**(-15/20.))
